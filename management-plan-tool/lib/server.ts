...
export async function authenticate(authorization: string | null): Promise<Identity> {
  ensure(authorization?.startsWith('Bearer '), 'Sign in to continue.', 401); const token = authorization.slice(7); ensure(token.length > 0 && token.length < 10000, 'Invalid session.', 401);
