...
export interface Review extends Sensitive { id: string; meetingId: string; scheduledFor: string; status: 'scheduled' | 'completed'; notes: string; outcome: 'no_change' | 'update_plan' | 'follow_up' | 'escalation' | ''; nextReview: string; decisionId: string; completedAt: string; sheet: { expectationIds: string[]; feedbackIds: string[]; actionIds: string[]; followUpIds: string[] } }
...
export interface View extends Omit<State, 'members'> { me: Member; members: Member[]; directory: { id: string; name: string; jobTitle: string }[] }
